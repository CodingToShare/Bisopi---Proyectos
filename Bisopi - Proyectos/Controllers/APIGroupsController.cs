...
namespace Bisopi___Proyectos.Controllers
{
    ...
    public class APIGroupsController : Controller
    {
        ...
        public APIGroupsController(DataContext context) {
            _context = context;
        }

        [HttpGet]
        public async Task<IActionResult> Get(DataSourceLoadOptions loadOptions) {
            var groups = _context.Groups.Select(i => new {
                i.Id,
                i.Name,
                i.IsActive,
                i.CreatedBy,
        ...
        public async Task<IActionResult> Put(Guid key, string values) {
            var model = await _context.Groups.FirstOrDefaultAsync(item => item.Id == key);
            if(model == null)
        ...
        private void PopulateModel(Group model, IDictionary values) {
            string ID = nameof(Group.Id);
            string NAME = nameof(Group.Name);
            string IS_ACTIVE = nameof(Group.IsActive);
            string CREATED_BY = nameof(Group.CreatedBy);
            string CREATED = nameof(Group.Created);
            string MODIFIED_BY = nameof(Group.ModifiedBy);
            string MODIFIED = nameof(Group.Modified);
